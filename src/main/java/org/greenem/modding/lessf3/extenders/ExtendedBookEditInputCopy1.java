package org.greenem.modding.lessf3.extenders;
...
public class ExtendedBookEditInputCopy1 extends Screen {
    ...
    private static final Component EDIT_TITLE_LABEL = new TextComponent("Text3");
    private static final Component FINALIZE_WARNING_LABEL = new TextComponent("Text4");
    ...
    @Nullable
    private ExtendedBookEditInputCopy1.DisplayCache displayCache = ExtendedBookEditInputCopy1.DisplayCache.EMPTY;
    private Component pageMsg = TextComponent.EMPTY;
    private Component ownerText = null;

    public ExtendedBookEditInputCopy1(Player p_98076_, ItemStack p_98077_, InteractionHand p_98078_) {
        ...
        this.ownerText = (new TextComponent("(Text5) book.byAuthor " + p_98076_.getName())).withStyle(ChatFormatting.DARK_GRAY);
    }

    protected ExtendedBookEditInputCopy1(Component p_96550_) {
        super(p_96550_);
    }
    ...
    protected void init() {
        ...
        this.signButton = this.addRenderableWidget(new Button(this.width / 2 - 100, 196, 98, 20, new TextComponent("Text6"), (p_98177_) -> {
            this.isSigning = true;
        ...
        this.finalizeButton = this.addRenderableWidget(new Button(this.width / 2 - 100, 196, 98, 20, new TextComponent("Text7"), (p_98168_) -> {
            if (this.isSigning) {
    ...
    private void keyEnd() {
        ExtendedBookEditInputCopy1.DisplayCache ExtendedBookEditInput$displaycache = this.getDisplayCache();
        int i = this.pageEdit.getCursorPos();
        int j = ExtendedBookEditInput$displaycache.findLineEnd(i);
        this.pageEdit.setCursorPos(j, Screen.hasShiftDown());
    }
    ...
    public void render(PoseStack p_98104_, int p_98105_, int p_98106_, float p_98107_) {
        ...
        if (this.isSigning) {
            ...
            for(ExtendedBookEditInputCopy1.LineInfo ExtendedBookEditInput$lineinfo : ExtendedBookEditInput$displaycache.lines) {
                this.font.draw(p_98104_, ExtendedBookEditInput$lineinfo.asComponent, (float)ExtendedBookEditInput$lineinfo.x, (float)ExtendedBookEditInput$lineinfo.y, -16777216);
            }

            this.renderHighlight(ExtendedBookEditInput$displaycache.selection);
            this.renderCursor(p_98104_, ExtendedBookEditInput$displaycache.cursor, ExtendedBookEditInput$displaycache.cursorAtEnd);
        }

        super.render(p_98104_, p_98105_, p_98106_, p_98107_);
    }

    private void renderCursor(PoseStack p_98109_, ExtendedBookEditInputCopy1.Pos2i p_98110_, boolean p_98111_) {
    ...
    private ExtendedBookEditInputCopy1.Pos2i convertScreenToLocal(ExtendedBookEditInputCopy1.Pos2i p_98115_) {
        return new ExtendedBookEditInputCopy1.Pos2i(p_98115_.x - (this.width - 192) / 2 - 36, p_98115_.y - 32);
    }

    private ExtendedBookEditInputCopy1.Pos2i convertLocalToScreen(ExtendedBookEditInputCopy1.Pos2i p_98146_) {
        return new ExtendedBookEditInputCopy1.Pos2i(p_98146_.x + (this.width - 192) / 2 + 36, p_98146_.y + 32);
    }
    ...
    public boolean mouseClicked(double p_98088_, double p_98089_, int p_98090_) {
        if (super.mouseClicked(p_98088_, p_98089_, p_98090_)) {
            ...
            if (p_98090_ == 0) {
                ...
                ExtendedBookEditInputCopy1.DisplayCache ExtendedBookEditInput$displaycache = this.getDisplayCache();
                int j = ExtendedBookEditInput$displaycache.getIndexAtPosition(this.font, this.convertScreenToLocal(new ExtendedBookEditInputCopy1.Pos2i((int)p_98088_, (int)p_98089_)));
                if (j >= 0) {
    ...
    public boolean mouseDragged(double p_98092_, double p_98093_, int p_98094_, double p_98095_, double p_98096_) {
        if (super.mouseDragged(p_98092_, p_98093_, p_98094_, p_98095_, p_98096_)) {
            ...
            if (p_98094_ == 0) {
                ExtendedBookEditInputCopy1.DisplayCache ExtendedBookEditInput$displaycache = this.getDisplayCache();
                int i = ExtendedBookEditInput$displaycache.getIndexAtPosition(this.font, this.convertScreenToLocal(new ExtendedBookEditInputCopy1.Pos2i((int)p_98092_, (int)p_98093_)));
    ...
    private ExtendedBookEditInputCopy1.DisplayCache getDisplayCache() {
        if (this.displayCache == null) {
            this.displayCache = this.rebuildDisplayCache();
            this.pageMsg = new TextComponent("Text 8 " + this.currentPage + 1 + " " + this.getNumPages());
        }
    ...
    private ExtendedBookEditInputCopy1.DisplayCache rebuildDisplayCache() {
        String s = this.getCurrentPageText();
        if (s.isEmpty()) {
            return ExtendedBookEditInputCopy1.DisplayCache.EMPTY;
        } else {
            int i = this.pageEdit.getCursorPos();
            int j = this.pageEdit.getSelectionPos();
            IntList intlist = new IntArrayList();
            List<ExtendedBookEditInputCopy1.LineInfo> list = Lists.newArrayList();
            ...
            stringsplitter.splitLines(s, 114, Style.EMPTY, true, (p_98132_, p_98133_, p_98134_) -> {
                ...
                int l3 = k3 * 9;
                ExtendedBookEditInputCopy1.Pos2i ExtendedBookEditInput$pos2i1 = this.convertLocalToScreen(new ExtendedBookEditInputCopy1.Pos2i(0, l3));
                intlist.add(p_98133_);
                list.add(new ExtendedBookEditInputCopy1.LineInfo(p_98132_, s3, ExtendedBookEditInput$pos2i1.x, ExtendedBookEditInput$pos2i1.y));
            });
            int[] aint = intlist.toIntArray();
            boolean flag = i == s.length();
            ExtendedBookEditInputCopy1.Pos2i ExtendedBookEditInput$pos2i;
            if (flag && mutableboolean.isTrue()) {
                ExtendedBookEditInput$pos2i = new ExtendedBookEditInputCopy1.Pos2i(0, list.size() * 9);
            } else {
                int k = findLineFromPos(aint, i);
                int l = this.font.width(s.substring(aint[k], i));
                ExtendedBookEditInput$pos2i = new ExtendedBookEditInputCopy1.Pos2i(l, k * 9);
            }
            ...
            if (i != j) {
                ...
                if (j1 == k1) {
                    ...
                    for(int j3 = j1 + 1; j3 < k1; ++j3) {
                        ...
                        list1.add(this.createSelection(new ExtendedBookEditInputCopy1.Pos2i(0, j2), new ExtendedBookEditInputCopy1.Pos2i(k2, j2 + 9)));
                    }
            ...
            return new ExtendedBookEditInputCopy1.DisplayCache(s, ExtendedBookEditInput$pos2i, flag, aint, list.toArray(new ExtendedBookEditInputCopy1.LineInfo[0]), list1.toArray(new Rect2i[0]));
        }
    }
    ...
    private Rect2i createPartialLineSelection(String p_98120_, StringSplitter p_98121_, int p_98122_, int p_98123_, int p_98124_, int p_98125_) {
        ...
        ExtendedBookEditInputCopy1.Pos2i ExtendedBookEditInput$pos2i = new ExtendedBookEditInputCopy1.Pos2i((int)p_98121_.stringWidth(s), p_98124_);
        ExtendedBookEditInputCopy1.Pos2i ExtendedBookEditInput$pos2i1 = new ExtendedBookEditInputCopy1.Pos2i((int)p_98121_.stringWidth(s1), p_98124_ + 9);
        return this.createSelection(ExtendedBookEditInput$pos2i, ExtendedBookEditInput$pos2i1);
    }

    private Rect2i createSelection(ExtendedBookEditInputCopy1.Pos2i p_98117_, ExtendedBookEditInputCopy1.Pos2i p_98118_) {
        ExtendedBookEditInputCopy1.Pos2i ExtendedBookEditInput$pos2i = this.convertLocalToScreen(p_98117_);
        ExtendedBookEditInputCopy1.Pos2i ExtendedBookEditInput$pos2i1 = this.convertLocalToScreen(p_98118_);
        int i = Math.min(ExtendedBookEditInput$pos2i.x, ExtendedBookEditInput$pos2i1.x);
        int j = Math.max(ExtendedBookEditInput$pos2i.x, ExtendedBookEditInput$pos2i1.x);
        int k = Math.min(ExtendedBookEditInput$pos2i.y, ExtendedBookEditInput$pos2i1.y);
        int l = Math.max(ExtendedBookEditInput$pos2i.y, ExtendedBookEditInput$pos2i1.y);
        return new Rect2i(i, k, j - i, l - k);
    }

    @OnlyIn(Dist.CLIENT)
    static class DisplayCache {
        static final ExtendedBookEditInputCopy1.DisplayCache EMPTY = new ExtendedBookEditInputCopy1.DisplayCache("", new ExtendedBookEditInputCopy1.Pos2i(0, 0), true, new int[]{0}, new ExtendedBookEditInputCopy1.LineInfo[]{new ExtendedBookEditInputCopy1.LineInfo(Style.EMPTY, "", 0, 0)}, new Rect2i[0]);
        private final String fullText;
        final ExtendedBookEditInputCopy1.Pos2i cursor;
        final boolean cursorAtEnd;
        private final int[] lineStarts;
        final ExtendedBookEditInputCopy1.LineInfo[] lines;
        final Rect2i[] selection;

        public DisplayCache(String p_98201_, ExtendedBookEditInputCopy1.Pos2i p_98202_, boolean p_98203_, int[] p_98204_, ExtendedBookEditInputCopy1.LineInfo[] p_98205_, Rect2i[] p_98206_) {
        ...
        public int getIndexAtPosition(Font p_98214_, ExtendedBookEditInputCopy1.Pos2i p_98215_) {
            int i = p_98215_.y / 9;
            if (i < 0) {
                return 0;
                ...
                ExtendedBookEditInputCopy1.LineInfo ExtendedBookEditInput$lineinfo = this.lines[i];
                return this.lineStarts[i] + p_98214_.getSplitter().plainIndexAtWidth(ExtendedBookEditInput$lineinfo.contents, p_98215_.x, ExtendedBookEditInput$lineinfo.style);
            }
        }

        public int changeLine(int p_98211_, int p_98212_) {
            int i = ExtendedBookEditInputCopy1.findLineFromPos(this.lineStarts, p_98211_);
            int j = i + p_98212_;
            int k;
        ...
        public int findLineStart(int p_98209_) {
            int i = ExtendedBookEditInputCopy1.findLineFromPos(this.lineStarts, p_98209_);
            return this.lineStarts[i];
        }

        public int findLineEnd(int p_98219_) {
            int i = ExtendedBookEditInputCopy1.findLineFromPos(this.lineStarts, p_98219_);
